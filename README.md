...
The `sqlite-mcp` and `pg-mcp` casks provide the upstream four-target release matrices: GNU/glibc Linux x86_64 and aarch64, and macOS Intel and Apple silicon. `sqlite-mcp`'s upstream release validation specifically names Ubuntu 24.04 and macOS 15. Do not infer support for other Linux distributions, older macOS versions, or unlisted architectures from target triples alone. `pg-mcp` installs the `postgres-mcp` executable.
...
For an enrolled project, a published stable release may trigger an automatic commit that updates only that project's version and four pinned SHA-256 values in `Casks/<token>.rb`. The casks live in `Casks/` and are what Homebrew installs; they are distinct from the inventory allowlist. `typedb-mcp` is not yet enrolled: its repository has stable Git tags but no published GitHub Release assets, so it cannot meet the tap's checksummed binary-release contract. Add it only after the producer publishes the exact four target archives, executable archive member, and `SHA256SUMS`, then review its platform claims and producer verification workflow. These bot commits are intentional: pinned versions and checksums let Homebrew discover and install upgrades conventionally. Duplicate releases are no-ops, older releases cannot roll a cask back, and a daily reconciliation recovers missed notifications.
...
Producer projects are not configured by this tap-only change. To opt in later, a producer maintainer must load `.agents/skills/homebrew-tools-release/SKILL.md` from this tap checkout and separately update their own release workflow after release publication and all release verification jobs succeed. The source repository's ordinary `GITHUB_TOKEN` is not assumed to authorize a cross-repository event. Configure a GitHub App installation token if possible, or a fine-grained token limited to this tap with **Contents: write** (Metadata: read is implicit), in the producer repository secret `HOMEBREW_TOOLS_DISPATCH_TOKEN`. Fine-grained tokens need an expiry and rotation plan. In sqlite-mcp and pg-mcp, dispatch must depend on the successful `publish`, `verify-published`, and `verify-release-metadata` jobs; pg-mcp's `verify-published` job runs a native smoke test for each target. Never pass credentials in the dispatch payload or URL.

The API call is `POST https://api.github.com/repos/tilmon-engineering/homebrew-tools/dispatches` with `event_type: "homebrew-tools-release"` and `client_payload: {"repository": "tilmon-engineering/<enrolled-repo>", "tag": "vX.Y.Z"}`. A failed dispatch fails the producer notification job but does not undo its already-published release; fix authorization and rerun that job or use this tap's manual replay. `workflow_dispatch` can replay a project/tag, while the scheduled reconciliation finds the latest stable release for every enrolled project.
...
`update-cask.yml` can be run manually for either enrolled token (`sqlite-mcp` or `pg-mcp`) and an exact release tag; it validates the published release from the GitHub API before any write. Scheduled reconciliation is the automatic recovery path for a missed dispatch. It does not modify the inventory, skill, or unrelated casks.
